...
public class CalendarUI extends JFrame {
    ...
    private final Color GREY = new Color(240, 240, 240);

    private LocalDate now = LocalDate.now();
    private HoverButton.handleMouseHover<JLabel> dayHoverHandler = new HoverButton.handleMouseHover<>();
    ...
    private class dayClickAdapter extends MouseAdapter {
        @Override
        public void mouseClicked(MouseEvent e) {
            super.mouseClicked(e);
            JLabel lb = (JLabel) e.getSource();

            now = now.withDayOfMonth(Integer.parseInt(lb.getText()));
            resetDate();
            // Todo: 单击切换过后，鼠标移出会导致背景色消失，但单击已经高亮的却不会，不知如何解决
        }
    }

    private dayClickAdapter dayClickHandler = new dayClickAdapter();
    ...
    private void setDayLabel(JLabel lb) {
        lb.setForeground(Color.BLACK);
        lb.addMouseListener(dayHoverHandler);
        lb.addMouseListener(dayClickHandler);
    }
    ...
    private void resetDate() {
        ...
        for (Component p : jpDate.getComponents()) {
            ...
            for (Component l : pa.getComponents()) {
                ...
                lb.removeMouseListener(dayHoverHandler);
                lb.removeMouseListener(dayClickHandler);
    ...
    private void createUIComponents() {
        btnExit = new HoverButton(Color.WHITE);
        prevButton = new HoverButton(Color.WHITE);
        nextButton = new HoverButton(Color.WHITE);
    }
